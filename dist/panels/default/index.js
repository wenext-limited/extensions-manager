...
module.exports = Editor.Panel.define({
    ...
    methods: {
        ...
        async _doRefreshList() {
            const showAll = this.$.showAll.checked;
            ...
            try {
                ...
                const extensions = await Editor.Message.request('extensions-manager', msgName);
        ...
        async loadVersionsForSelect(name, selectEl, currentVersion) {
            try {
                const tags = await Editor.Message.request('extensions-manager', 'fetch-tags', name);
    ...
    ready() {
        ...
        this.$.showAll.addEventListener('change', () => {
            this.refreshList();
        });
        ...
        // 显示插件自身版本号
        const titleEl = document.querySelector('.toolbar-left h2');
        if (titleEl)
            titleEl.textContent = `wenext扩展管理器 v${pkgJson.version}`;
        // ── 右键菜单初始化 ──
        ...
        this.$.ctxOpenDir.addEventListener('click', (e) => {
            ...
            Editor.Message.request('extensions-manager', 'open-extension-dir', name)
                ...
                this.log(`⚠ ${result.output}`); })
                .catch((err) => this.log(`⚠ 打开目录失败: ${err.message || err}`));
        });
        // 初始加载：先拉取远程注册表，再刷新列表
