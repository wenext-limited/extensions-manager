 ...
 */
function normalizePkgVersion(raw: unknown): string | null {
...
function getInstalledExtensionFolderNames(): string[] {
    ...
    for (const ent of fs.readdirSync(root, { withFileTypes: true })) {
        ...
        if (fs.existsSync(pkgPath)) out.push(name);
    }
...
interface ExtensionInfo {
    ...
    description: string;
    git: string;
    /** 保留字段：不再使用项目 extensions.json，恒为 null */
    requiredVersion: string | null;
    /** 来自 extensions/<name>/package.json 的 version（已规范化） */
...
function parseRemoteTagsFromLsRemote(stdout: string): string[] {
    const tags: string[] = [];
    const seen = new Set<string>();
    for (const rawLine of stdout.split('\n')) {
        ...
        if (tag.endsWith('^{}')) tag = tag.slice(0, -3);
...
async function fetchRemoteTags(gitUrl: string): Promise<string[]> {
    ...
    for (const cmd of tries) {
        try {
            ...
            console.warn(`[extensions-manager] 远程 tag 查询失败: ${cmd} -> ${err?.message || err}`);
        }
    }
    return [];
}

function parseSemverTuple(tag: string): [number, number, number] | null {
    const m = tag.trim().match(/^v?(\d+)\.(\d+)\.(\d+)$/);
    if (!m) return null;
    return [Number(m[1]), Number(m[2]), Number(m[3])];
}

function sortRemoteTags(tags: string[]): string[] {
...
export const methods: { [key: string]: (...args: any) => any } = {
    ...
    async querySelfInfo(): Promise<ExtensionInfo> {
        ...
        const installedVersion = normalizePkgVersion(packageJSON.version as unknown);
